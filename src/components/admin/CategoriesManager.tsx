...
export default function CategoriesManager({ initialCategories }: Props) {
  ...
  async function handleCategorySubmit(e: Event) {
    ...
    try {
      ...
      if (editingCategory) {
        setCategories(cats =>
          cats.map(c => c.id === data.category.id ? { ...data.category, subcategories: c.subcategories } : c)
        );
        setSuccessMessage("Categoría actualizada");
      } else {
        setCategories(cats => [...cats, { ...data.category, subcategories: [] }]);
  ...
  async function handleSubcategorySubmit(e: Event) {
    ...
    try {
      // Process filter_config
      const processedFilters: FilterConfig[] = subcategoryForm.filter_config.map(f => {
      ...
      // Update categories state with new/updated subcategory
  ...
  async function handleDeleteSubcategory() {
    ...
    try {
      const response = await fetch(`/api/admin/subcategories/${deleteModal.id}`, {
        method: "DELETE",
      });
      ...
      setCategories(cats => cats.map(cat => ({
        ...cat,
        subcategories: cat.subcategories.filter(sub => sub.id !== deleteModal.id),
      })));
    ...
    <div class={styles.manager}>
      ...
      <div class={styles.header}>
        <div class={styles.tabs}>
          <button
            class={`${styles.tab} ${activeTab === "categories" ? styles.activeTab : ""}`}
          ...
          <button
            class={`${styles.tab} ${activeTab === "subcategories" ? styles.activeTab : ""}`}
            ...
            <div key={category.id} class={styles.card}>
              <div class={styles.cardHeader}>
                ...
                <div class={styles.cardStats}>
                  <span class={styles.statNumber}>{category.subcategories.length}</span>
                  <span class={styles.statLabel}>subs</span>
                </div>
              </div>
              
              <div class={styles.cardActions}>
                <button class={styles.editBtn} onClick={() => openCategoryModal(category)}>
                ...
                <button
                  ...
                  title={category.subcategories.length > 0 ? "Elimina las subcategorías primero" : "Eliminar categoría"}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div key={sub.id} class={styles.tableRow}>
              ...
              <div class={styles.filtersBadges}>
                {sub.filter_config.length > 0 ? (
                  <>
                    <span class={styles.filterCount}>{sub.filter_config.length} filtros</span>
                    <div class={styles.filtersList}>
                      {sub.filter_config.map(f => (
                        <span key={f.key} class={styles.filterChip} title={`Tipo: ${f.type}`}>
                          {f.label}
                        </span>
                      ))}
                    </div>
              ...
              <div class={styles.rowActions}>
                <button class={styles.editBtn} onClick={() => openSubcategoryModal(sub)}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                  </svg>
        ...
        <div class={styles.modalOverlay} onClick={() => setShowCategoryModal(false)}>
          <div class={styles.modal} onClick={e => e.stopPropagation()}>
            <div class={styles.modalHeader}>
              ...
              <button class={styles.closeModal} onClick={() => setShowCategoryModal(false)}>
            ...
            <form onSubmit={handleCategorySubmit}>
              ...
              <div class={styles.formGroup}>
                ...
                <input
                  ...
                  onInput={e => setCategoryForm(f => ({ ...f, slug: e.currentTarget.value }))}
                  placeholder={generateSlug(categoryForm.name) || "se-genera-automaticamente"}
                />
                <span class={styles.hint}>Se genera automáticamente si lo dejas vacío</span>
              </div>
              ...
              <div class={styles.formGroup}>
                ...
                <input
                  ...
                  onInput={e => setCategoryForm(f => ({ ...f, image_url: e.currentTarget.value }))}
                  placeholder="https://..."
                />
              </div>
              
              <div class={styles.modalActions}>
                <button type="button" class={styles.cancelBtn} onClick={() => setShowCategoryModal(false)}>
                  Cancelar
                </button>
      ...
      {showSubcategoryModal && (
        <div class={styles.modalOverlay} onClick={() => setShowSubcategoryModal(false)}>
          <div class={`${styles.modal} ${styles.largeModal}`} onClick={e => e.stopPropagation()}>
            <div class={styles.modalHeader}>
              <h2>{editingSubcategory ? "Editar Subcategoría" : "Nueva Subcategoría"}</h2>
              <button class={styles.closeModal} onClick={() => setShowSubcategoryModal(false)}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <line x1="18" y1="6" x2="6" y2="18" />
                  <line x1="6" y1="6" x2="18" y2="18" />
                </svg>
            ...
            <form onSubmit={handleSubcategorySubmit}>
              <div class={styles.formRow}>
                <div class={styles.formGroup}>
                  ...
                  <input
                    ...
                    onInput={e => setSubcategoryForm(f => ({ ...f, name: e.currentTarget.value }))}
                ...
                <div class={styles.formGroup}>
                  ...
                  <input
                    ...
                    onInput={e => setSubcategoryForm(f => ({ ...f, slug: e.currentTarget.value }))}
                    placeholder={generateSlug(subcategoryForm.name) || "automatico"}
                  />
                </div>
              </div>
              ...
              <div class={styles.formRow}>
                <div class={styles.formGroup}>
                  ...
                  <select
                    ...
                    onChange={e => setSubcategoryForm(f => ({ ...f, category_id: e.currentTarget.value }))}
                    required
                  >
                ...
                <div class={styles.formGroup}>
                  ...
                  <input
                    ...
                    onInput={e => setSubcategoryForm(f => ({ ...f, display_order: parseInt(e.currentTarget.value) || 0 }))}
                    min={0}
                  />
                </div>
              </div>
              ...
              <div class={styles.filtersSection}>
                <div class={styles.filtersSectionHeader}>
                  ...
                  <p>Define los atributos filtrables para los productos de esta subcategoría</p>
                </div>
                    ...
                    <div class={styles.filterRow}>
                      <div class={styles.formGroup}>
                        ...
                        <input
                          type="text"
                          value={filter.key}
                          onInput={e => updateFilter(index, "key", e.currentTarget.value)}
                          placeholder="ej: size"
                          required
                        />
                      </div>
                      ...
                      <div class={styles.formGroup}>
                        ...
                        <input
                          type="text"
                          value={filter.label}
                          onInput={e => updateFilter(index, "label", e.currentTarget.value)}
                      ...
                      <div class={styles.formGroup}>
                        ...
                        <select
                          value={filter.type}
                          onChange={e => updateFilter(index, "type", e.currentTarget.value)}
                        >
                    ...
                    {(filter.type === "select" || filter.type === "checkbox") && (
                      <div class={styles.filterOptions}>
                        ...
                        <input
                          ...
                          onInput={e => updateFilter(index, "tempOptions", e.currentTarget.value)}
                      ...
                      <div class={styles.rangeOptions}>
                        <div class={styles.formGroup}>
                          ...
                          <input
                            type="number"
                            value={filter.min}
                            onInput={e => updateFilter(index, "min", parseFloat(e.currentTarget.value))}
                          />
                        </div>
                        <div class={styles.formGroup}>
                          <label>Máximo</label>
                          <input
                            type="number"
                            value={filter.max}
                            onInput={e => updateFilter(index, "max", parseFloat(e.currentTarget.value))}
                          />
                        </div>
                        <div class={styles.formGroup}>
                          <label>Paso</label>
                          <input
                            type="number"
                            value={filter.step}
                            onInput={e => updateFilter(index, "step", parseFloat(e.currentTarget.value))}
                            step="any"
                          />
                        </div>
                      </div>
              ...
              <div class={styles.modalActions}>
                <button type="button" class={styles.cancelBtn} onClick={() => setShowSubcategoryModal(false)}>
                  Cancelar
                </button>
                <button type="submit" class={styles.submitBtn} disabled={isLoading}>
      ...
      {deleteModal && (
        <div class={styles.modalOverlay} onClick={() => setDeleteModal(null)}>
          <div class={styles.deleteModalContent} onClick={e => e.stopPropagation()}>
            ...
            <h3>Eliminar {deleteModal.type === "category" ? "Categoría" : "Subcategoría"}</h3>
            <p>
              ¿Estás seguro de que deseas eliminar <strong>{deleteModal.name}</strong>?
              Esta acción no se puede deshacer.
            </p>
            <div class={styles.modalActions}>
              <button class={styles.cancelBtn} onClick={() => setDeleteModal(null)}>
                Cancelar
              </button>
              <button
                class={styles.dangerBtn}
                onClick={deleteModal.type === "category" ? handleDeleteCategory : handleDeleteSubcategory}
                disabled={isLoading}
              >
