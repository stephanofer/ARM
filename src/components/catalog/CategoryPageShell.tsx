...
import type { Category, Subcategory, Product, FilterConfig } from '../../lib/data';
import {
  filtersStore,
  initFiltersStore,
  productsStore,
...
export function CategoryPageShell({
  ...
  const [currentFilterConfig, setCurrentFilterConfig] = useState<FilterConfig[]>(filterConfig);
  ...
  const displayTotalPages = isInitialized ? products.totalPages : initialPagination.totalPages;
  ...
  useEffect(() => {
    ...
    const subcategory = subcategories.find(s => s.slug === filters.subcategorySlug);
    if (subcategory) {
  ...
  useEffect(() => {
    ...
    const buildURL = () => {
      ...
      return `/categorias/${category.slug}${queryString ? '?' + queryString : ''}`;
    };
    ...
    const fetchProducts = async () => {
      try {
        ...
        productsStore.set({ ...productsStore.get(), isLoading: true, error: null });
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      const reservedParams = ['subcategoria', 'page', 'sort', 'minPrice', 'maxPrice', 'inStock'];
    ...
    <div className={styles.container}>
      ...
      <div className={styles.mainContent}>
        ...
        <main className={styles.content}>
          ...
          {!products.error && !products.isLoading && displayProducts.length === 0 && (
            <EmptyState />
          )}
